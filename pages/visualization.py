...
with tab3:
    ...
    if categorical_columns:
        ...
        with col2:
            violin_opacity = st.slider("Tranparenz", min_value=0.0, max_value=1.0, value=0.4, step=0.05)
        ...
        st.info("**Tipp:** Wenn die Violinen im rechten Plot fast identisch sind, "
...
with tab4:
    st.title("Korrelation aller Attribute mit dem Exam Score")
    df_encoded = df.copy()
    cat_cols = df_encoded.select_dtypes(include=['object', 'category']).columns
    for col in cat_cols:
        df_encoded[col] = df_encoded[col].astype('category').cat.codes
    
    correlations = df_encoded.corr()[['exam_score']].sort_values(by='exam_score', ascending=False)
